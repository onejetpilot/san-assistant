...
def test_stream_answer_saves_full_final_answer(monkeypatch):
    saved: dict = {}
    monkeypatch.setattr('app.services.answer_service.save_chat_request', lambda **k: saved.update(k))
    monkeypatch.setattr('app.services.answer_service.save_knowledge_gap', lambda **k: None)
    monkeypatch.setattr('app.services.answer_service.get_current_index_versions', lambda: {})
    ...
    service.rag = type('R', (), {
        'available': True,
        'search': lambda self, q, preferred_doc_id=None: [
            RetrievedChunk(
                text='TECHNICAL SPECIFICATIONS\nНоминальное давление: 1.6 МПа',
                metadata={'doc_id': 'ondo_axial', 'section': 'TECHNICAL SPECIFICATIONS', 'section_group': 'technical'},
                score=0.8,
    ...
    assert any(item['event'] == 'done' for item in events)
    assert saved['answer'] == 'Полный ответ'


async def _collect_stream(service: AnswerService):
    out = []
    async for event in service.answer_stream('Расскажи о товаре'):
        out.append(event)
    return out
